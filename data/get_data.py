...
def get_graph_data(rsmi_list, yld_list, data_id, filename):
    ...
    elif data_id == 4: atom_list = ['C', 'O', 'N', 'Cl', 'S', 'F', 'P'] 
    elif data_id == 5: atom_list = ['C', 'N', 'O', 'F', 'Si', 'P', 'S', 'Cl', 'Br']
    
    charge_list = [1, 2, -1, 0]
    
    if data_id in [1, 4, 5, 6]: degree_list = [1, 2, 3, 4, 0]
    else: degree_list = [1, 2, 3, 4, 5, 6, 0] 
    
    if data_id in [1, 4, 5]: valence_list = [1, 2, 3, 4, 5, 6, 0]   
    else: valence_list = [1, 2, 3, 4, 5, 6, 7, 0]
    
    hybridization_list = ['SP','SP2','SP3','SP3D','SP3D2','S','UNSPECIFIED']
    hydrogen_list = [1, 2, 3, 0]
    ringsize_list = [3, 4, 5, 6, 7, 8]
    if data_id == 1 or 4: bond_list = ['SINGLE', 'DOUBLE', 'TRIPLE', 'AROMATIC', 'DATIVE']
    else: bond_list = ['SINGLE', 'DOUBLE', 'TRIPLE', 'AROMATIC']
...
if __name__ == "__main__":
    ...
    for data_id in [1, 4, 5, 6, 7]:
        # for sub in range(1, 3):
        for split_id in range(10):

            load_dict = np.load('./split/data_%d_split_%d.npz' %(data_id, split_id), allow_pickle = True)
            rsmi_list = load_dict['data_df'][:,0]
            yld_list = load_dict['data_df'][:,1]
            filename = './intermediate/dataset_%d_%d.npz' %(data_id, split_id)
