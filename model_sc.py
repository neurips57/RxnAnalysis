...
class reactionMPNN(nn.Module):
    ...
    def __init__(self, node_in_feats, edge_in_feats, n_rmol, n_imol,
        ...
        self.predict = [nn.Sequential(
            nn.Linear((readout_feats+self.one_hot_len) * i, predict_hidden_feats), nn.PReLU(), nn.Dropout(prob_dropout),
            nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(), nn.Dropout(prob_dropout))
            .to(cuda) for i in range(5 + 1, self.one_hot_len+1)]
    
        self.predict2 = nn.Sequential(
            nn.Linear(predict_hidden_feats * (n_imol+1), predict_hidden_feats), nn.PReLU(), nn.Dropout(prob_dropout),
            nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(), nn.Dropout(prob_dropout))
    ...
    def forward(self, rmols, imols, pmols):
        ...
        '''
        self.mpnn - GNN features are extracted
        torch.cat - concataned with one hot vector
        '''
        batch_sz = self.mpnn(rmols[0]).size(0)
        one_hot_len = 5 + self.n_imol + 1
        ...
        for i in range(5):
            ...
            if i==3:
                base_graph = torch.sum(torch.stack([self.mpnn(mol) for mol in rmols[3:self.n_rmol-1]]), 0)
                r_graph = torch.reshape(torch.cat([base_graph, one_hot_tensor], 1),(1,batch_sz ,d_model))
                
            if i>3:
                sol_graph = torch.sum(torch.stack([self.mpnn(mol) for mol in rmols[self.n_rmol-1:]]), 0)
                r_graph = torch.reshape(torch.cat([sol_graph, one_hot_tensor], 1),(1,batch_sz ,d_model))
            else:
        ...
        for i in range(self.n_imol):
            one_hot = [[i + 5 == j for j in range(one_hot_len)] for _ in range(batch_sz)]
            one_hot_tensor = torch.Tensor(one_hot).to(cuda)
        ...
        '''
        r = [torch.reshape(out[i],(batch_sz ,d_model)) for i in range(5)]
        it = [torch.reshape(out[i],(batch_sz ,d_model)) for i in range(5,5+self.n_imol)]
        p = torch.reshape(out[5+self.n_imol],(batch_sz ,d_model)) 
        it_p = it+[p]
        delta_e = []
        for i in range(1,self.n_imol+2):
