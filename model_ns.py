...
class reactionMPNN(nn.Module):
    ...
    def __init__(self, node_in_feats, edge_in_feats, n_rmol, n_imol,
        ...
        self.one_hot_len = n_rmol + int(n_imol/2) + 1
        self.d_model = self.readout_feats + self.one_hot_len
        self.h_bond = nn.Sequential(
            nn.Linear((readout_feats) * (2), predict_hidden_feats), nn.PReLU(), nn.Dropout(prob_dropout),
            nn.Linear(predict_hidden_feats, readout_feats), nn.PReLU(), nn.Dropout(prob_dropout)
        )
        
        self.encoder_layer = nn.TransformerEncoderLayer(d_model = self.d_model, nhead = 8).to(cuda)

        self.predict = [nn.Sequential(
            nn.Linear((readout_feats + n_rmol + int(n_imol/2) + 1) * i, predict_hidden_feats), nn.PReLU(), nn.Dropout(prob_dropout),
            nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(), nn.Dropout(prob_dropout))
            .to(cuda) for i in range(n_rmol + 1, n_rmol + int(n_imol/2) + 2)]
    
        self.predict2 = nn.Sequential(
            nn.Linear(predict_hidden_feats * (int(n_imol/2)+1), predict_hidden_feats), nn.PReLU(), nn.Dropout(prob_dropout),
            nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(), nn.Dropout(prob_dropout))
    ...
    def forward(self, rmols, imols, pmols):
        ...
        one_hot_len = self.n_rmol + int(self.n_imol/2) + 1
        ...
        i_graph_feats = []
        for i in range(int(self.n_imol/2)):
            one_hot = [[i + self.n_rmol == j for j in range(one_hot_len)] for _ in range(batch_sz)]
            one_hot_tensor = torch.Tensor(one_hot).to(cuda)
            h_bonding = self.h_bond(torch.cat([self.mpnn(imols[2*i]), self.mpnn(imols[2*i+1])], 1))
            i_graph = torch.reshape(torch.cat([h_bonding, one_hot_tensor], 1), (1, batch_sz, d_model))
        ...
        it = [torch.reshape(out[i],(batch_sz ,d_model)) for i in range(self.n_rmol,self.n_rmol+int(self.n_imol/2))]
        p = torch.reshape(out[self.n_rmol+int(self.n_imol/2)],(batch_sz ,d_model)) 
        it_p = it+[p]
        delta_e = []
        for i in range(1,int(self.n_imol/2)+2):
